...
def make_report(invalid_deliveries, reasons, max_weight=10):
    with open("invalid_deliveries_report.txt", "w") as report_file:
        report_file.write(
            f"========================================\n"
            f"INVALID DELIVERY REPORT\n"
            f"========================================\n"
            f"Vehicle capacity: {max_weight:g} kg\n"
            f"Invalid deliveries: {len(invalid_deliveries)}\n\n"
        )
        for delivery, reason in zip(invalid_deliveries, reasons):
            report_file.write(
                f"Delivery ID: {delivery.ID}, Area: {delivery.area}, "
                f"Priority: {delivery.priority}, Weight: {delivery.weight} kg\n\n"
            )
